  ...
  template<typename _Tp>
    struct _Rb_tree_iterator
    {
      ...
      ~_Rb_tree_iterator(){
    	if(_M_node){
			_M_node->_TZ_refCount -= 1;
			if(_M_node->_TZ_refCount == 0 && _M_node->_TZ_destroy_node){
				(_M_node->*(_M_node->_TZ_destroy_node))(_M_node);
			}
    	}
      }
  ...
  template<typename _Tp>
    struct _Rb_tree_const_iterator
    {
      ...
      ~_Rb_tree_const_iterator(){
    	  ((_Rb_tree_node_base*)_M_node)->_TZ_refCount -= 1;
      	if(_M_node->_TZ_refCount == 0 && _M_node->_TZ_destroy_node){
      		(((_Rb_tree_node_base*)_M_node)->*(((_Rb_tree_node_base*)_M_node)->_TZ_destroy_node))((void*)_M_node);
      	}
      }
...
//			else
				__p->_M_parent = 0;
			__p->_M_color = _S_red;
			__p->_TZ_destroy_node = reinterpret_cast<void(_Rb_tree_node_base::*)(void*)>(&_Rb_tree_impl::_TZ_simple_delete);
		}
	  }
